...
class MinimaxBehaviour():
    ...
    def get_velocity_and_omega(self, own_pos: PosAndOrientation, mouse_tracker: MouseTracker):
        ...
        self.current_enemy_properties = mouse_tracker.get_mouse_capabilities() # enemy capabilities could incorporate more info than just properties in the future, but for now just save it as is
        self.current_enemy_choices = np.linspace(-self.current_enemy_properties.max_omega, self.current_enemy_properties.max_omega, self.choices)

        all_move_values = [self._minimax(choice, own_pos, other_pos, self.tree_depth, mean_enemy_vel, cheese_target, True) for choice in self.strategy_choices]
    ...
    def _minimax(self, angular_vel: float, own_pos: PosAndOrientation, other_pos: PosAndOrientation, mean_enemy_vel: float, cheese_target: Positition2D, depth: int, optimize_for_cat: bool, alpha: float = -np.inf, beta: float = np.inf):
        ...
        if optimize_for_cat:            
            ...
            for choice in self.strategy_choices:
                new_value = self._minimax(choice, new_own_pos, new_other_pos, cheese_target, depth-1, False, alpha, beta)
            ...
            for choice in self.current_enemy_choices:
                new_value = self._minimax(choice, new_own_pos, new_other_pos, cheese_target, depth-1, True, alpha, beta)
    ...
    def _cat_metric(self, own_pos: PosAndOrientation, other_pos: PosAndOrientation, cheese_target: Positition2D):
        ...
        mouse_cheese_distance = get_distance_between_positions(other_pos.pos, cheese_target)
        ...
        return distance_weight * distance + angle_weight * (1 - np.abs(0.5 - angle)) + mouse_cheese_distance
    ...
    def __system_update(self, angular_vel: float, own_pos: PosAndOrientation, other_pos: PosAndOrientation, mean_enemy_vel:float, optimize_for_cat: bool):
        ...
        if optimize_for_cat:
            ...
            speed = mean_enemy_vel #  maybe the mouse does not alwayse use max speed? use a rolling average over the last few velocities
