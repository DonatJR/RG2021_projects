...
class CollisionAvoidanceBehaviour():
    ...
    def __collision_avoidance(self, ranges, angles):
        if len(ranges) == 0: # can happen when all obstacles are further away than laser can scan
            omega = 0
        elif (ranges[np.logical_or(angles > self.ahead_bounds[0], angles < self.ahead_bounds[1])] > self.distance_to_obstacle).all(): # if distance to obstacle is big, just drive forward
            omega = 0
        else:
            # normalize ranges 
            norm_ranges = (ranges - np.min(ranges)) / (np.max(ranges) - np.min(ranges) + 0.01)
