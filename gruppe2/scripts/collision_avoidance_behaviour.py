...
class CollisionAvoidanceBehaviour():
    ...
    def __collision_avoidance(self, ranges, angles):
        fov_ranges = ranges[np.logical_or(angles > self.ahead_bounds[0], angles < self.ahead_bounds[1])]
        fov_angles = angles[np.logical_or(angles > self.ahead_bounds[0], angles < self.ahead_bounds[1])]

        if len(ranges) == 0: # can happen when all obstacles are further away than laser can scan
            omega = 0
        elif (fov_ranges > self.distance_to_obstacle).all(): # if distance to obstacle is big, just drive forward
            omega = 0
        else:
            # normalize ranges
            norm_ranges = (fov_ranges - np.min(fov_ranges)) / (np.max(fov_ranges) - np.min(fov_ranges) + 0.01)

            # compute the angular command velocity omega (turning rate)
            k = 0.001
            force = (4/(norm_ranges + 0.01) - 1) * k 
            force_angles = fov_angles - np.pi # opposite force direction than sensor direction
