...
class Cat:
    def __init__(self, self_cmd_vel_topic, self_odom_topic, self_scan, enemy_odom_topic, Behaviour) -> None:
        ...
        # Topics
        rospy.Subscriber(self_odom_topic, Odometry,
                         lambda odom: self.__odom_callback(odom, True))
        rospy.Subscriber(enemy_odom_topic, Odometry,
                         lambda odom: self.__odom_callback(odom, False))
        rospy.Subscriber(self_scan, LaserScan, self.__scan_callback)

        self.cmd_vel_pub = rospy.Publisher(
            self_cmd_vel_topic, Twist, queue_size=10)
    ...
    def __get_cheese_pos(self):
        ...
        cheese_1_middle = np.mean(np.load(os.path.join(
            catch_path, 'maps/cheese_' + str(1) + '.npy')), axis=0)[0]
        cheese_indices = [1, 2, 3, 4] if rogata_helper().inside(
            "cheese_obj", cheese_1_middle) else [5, 6, 7, 8]
    ...
    def __scan_callback(self, scan_msg):
        ...
        angles = np.arange(angle_min, angle_max +
    ...
    def __init_properties(self):
        ...
        self.properties = AnimalProperties(
            0.2 + random_velocity_factor, 0.8 + random_omega_factor)
    ...
    def __odom_callback(self, odom: Odometry, self_odom: bool):
        ...
        if self_odom:
            self.self_pos = PosAndOrientation(
                odom.pose.pose.position.x, odom.pose.pose.position.y, orientation)
            self.self_odom_callback_received = True
        else:
            self.mouse_tracker.update_position_and_orientation(PosAndOrientation(
                odom.pose.pose.position.x, odom.pose.pose.position.y, orientation))
            self.mouse_tracker.update_capabilities(
                odom.twist.twist.linear.x, odom.twist.twist.linear.z)
    ...
    def spin(self):
        while not rospy.is_shutdown():
            ...
            velocity, angle = self.behavior.get_velocity_and_omega(
                self.self_pos, self.mouse_tracker, self.scan)

            out = Twist()
            out.linear.x = np.clip(
                velocity, 0.2, self.properties.max_linear_vel)
            out.angular.z = np.clip(
                angle, -self.properties.max_omega, self.properties.max_omega)
...
if __name__ == '__main__':
    ...
    # sys.argv[5] # behaviour does only need to change for testing, so setting it here in the script directly is easier
    behaviour_to_use = 'combined'

    try:
        rospy.init_node('cat')
        cat = Cat(self_cmd_vel_topic, self_odom_topic, self_scan,
                  enemy_odom_topic, get_all_behaviours()[behaviour_to_use])
        cat.spin()
